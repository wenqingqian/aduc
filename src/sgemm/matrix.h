...
public:
	...
	matrix operator * (float alpha) const {
		matrix<T> nma(M, N, true, false);
		for(int i = 0; i < M; i ++){
			for(int j = 0; j < N; j ++){
				nma[i*N+j] = alpha * data[i*N+j];
			}
		}
		return nma;
	}
	...
	template< typename Ts >
	friend matrix<Ts> operator * (float alpha, const matrix<Ts>& ma) {
		return ma * alpha;
	}
	...
	static T diff(matrix& ma1, matrix& ma2){
		...
		for(int i = 0; i < ma1.M; i++){
			for(int j = 0; j < ma1.N; j++){
				ans += abs(ma1[i*ma1.N+j]-ma2[i*ma1.N+j])*100.0/abs(ma1[i*ma1.N+j]);
			}
		}
		return ans/(ma1.M*ma1.N);
	}
